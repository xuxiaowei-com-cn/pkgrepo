...
// Fetcher 负责读取仓库中的文件（repomd.xml、primary.xml 等）。
...
func (f *HTTPFetcher) Open(ctx context.Context, rawURL string) (io.ReadCloser, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("rpmrepo: 构造请求失败: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("rpmrepo: 请求 %s 失败: %w", rawURL, err)
	}
...
func filePath(rawURL string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("rpmrepo: 非法的地址 %q: %w", rawURL, err)
	}
	if u.Scheme == "" {
		return rawURL, nil
	}
	if u.Scheme != "file" {
		return "", fmt.Errorf("rpmrepo: 不支持的协议 %q", u.Scheme)
	}
	if u.Host != "" && u.Host != "localhost" {
		return "", fmt.Errorf("rpmrepo: 不支持的 file 地址主机 %q", u.Host)
	}
	if u.Path == "" {
		return "", fmt.Errorf("rpmrepo: file 地址缺少路径: %q", rawURL)
	}
	return u.Path, nil
}
...
func (f *schemeFetcher) Open(ctx context.Context, rawURL string) (io.ReadCloser, error) {
	u, err := url.Parse(rawURL)
	if err != nil {
		return nil, fmt.Errorf("rpmrepo: 非法的地址 %q: %w", rawURL, err)
	}
	...
	default:
		return nil, fmt.Errorf("rpmrepo: 不支持的协议 %q", u.Scheme)
	}
}
